...
// API: update post (merge provided fields into stored post) - used to persist comment likes/replies
app.post('/api/posts/:id/update', (req, res) => {
  ...
  const p = posts.find(x=>x.id===id);
  if(!p) return res.status(404).json({ error: 'not found' });
  // merge allowed fields (safeguard)
  const allowed = ['modalComments','likes','caption','location','coauthors','meta','filter'];
